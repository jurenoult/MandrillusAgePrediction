...
class SAM2UNet(nn.Module):
    def __init__(self, checkpoint_path=None) -> None:
        ...
        if checkpoint_path:
            print(f"Loading same with checkpoint path={checkpoint_path}")
        ...
        self.up3 = Up(128, 64)
        self.up4 = Up(128, 64)
        self.side1 = nn.Conv2d(64, 2, kernel_size=1)
        self.side2 = nn.Conv2d(64, 2, kernel_size=1)
        self.last = nn.Conv2d(64, 2, kernel_size=1)
        self.final_merge = nn.Conv2d(2, 2, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.up3(x, x1)
        out = F.interpolate(self.last(x), scale_factor=4, mode="bilinear")
