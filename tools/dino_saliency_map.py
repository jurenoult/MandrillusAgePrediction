...
def compute_raw_gradients(model, input_tensor):
    ...
    blurred = blur(input_tensor)
    attr = ig.attribute(input_tensor, baselines=blurred, target=None, n_steps=50)  # Regression → target=None
    return attr
...
def main(model_path, im_path, im_folder, dino_type, device):
    # Xor
    assert bool(im_path) != bool(im_folder), "Expected either image path or folder path but not both"
    print("Starting !")
