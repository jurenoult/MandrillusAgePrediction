...
def standard_regression_evaluation(y_true, y_pred, name, min_range, max_range):
    ...
    regression_results = evaluate_regression(y_true, y_pred, steps, min_range, max_range, regression_name)
    classification_name = f"{name}_as_classification"
    classification_results = evaluate_regression_as_classification(y_true, y_pred, steps, min_range, max_range, classification_name)
    
    return format_results(regression_name, regression_results, classification_name, classification_results, name)

def standard_classification_evaluation(y_true, y_prob_pred, class_step, n_classes, name):
    ...
    regression_results = evaluate_classification_as_regression(y_true, y_prob_pred, class_step, n_classes, regression_name)
    
    return format_results(regression_name, regression_results, classification_name, classification_results, name)
    
def format_results(regression_name, regression_results, classification_name, classification_results, name):
    return {
       name: {
...
def evaluate_regression(y_true, y_pred, steps, min_range, max_range, name):
    ...
    return {
        f"{name}_mae": global_mae,
        f"{name}_mae_steps": step_results
    }
...
def evaluate_classification(y_true, y_pred, class_step, n_classes):
...
def evaluate_classification_as_regression(y_true_classes, y_pred_prob_classes, class_step, n_classes, name):
    labels = range_to_labels(class_step, n_classes)
    y_true_scalars = classification_to_regression(y_true_classes, labels)
    y_pred_scalars = classification_to_regression(np.argmax(y_pred_prob_classes, axis=-1), labels)
    y_pred_weigthed_scalars = classification_prob_to_regression(y_pred_prob_classes, labels)
    min_range = 0
    max_range = class_step * n_classes
    return {
        "argmax": evaluate_regression(y_true_scalars, y_pred_scalars, [class_step, 90], min_range, max_range, name),
        "weighted_scalars": evaluate_regression(y_true_scalars, y_pred_weigthed_scalars, [class_step, 90], min_range, max_range, name)
    }

def evaluate_regression_as_classification(y_true_scalars, y_pred_scalars, class_steps, min_range, max_range, name):
    all_results = {}
    for class_step in class_steps:
        n_classes = int(np.ceil(max_range / class_step))
        current_name = f"{name}_{int(class_step)}"
        y_true_classes = regression_to_classification(y_true_scalars, class_step, min_range, max_range)
        y_pred_classes = regression_to_classification(y_pred_scalars, class_step, min_range, max_range)
